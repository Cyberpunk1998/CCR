...
parser.add_argument('--name', default='CCR_Model_SUES', type=str, help='output model name')
...
parser.add_argument('--model', default='convnext_base_22k_224', type=str, metavar='MODEL',
                    help='Name of model to train')
parser.add_argument('--epochs', default=80, type=int, help='')
        ...
        for phase in ['train']:
            ...
            for data, data2 in dataloaders:
                ...
                if phase == 'val':
                    ...
                    with autocast():
                        if opt.views == 2:
                            outputs, outputs2 = model(inputs, inputs2)  # satellite and drone
                            # print(len(outputs))
                        # elif opt.views == 3:
                ...
                if opt.views == 2:
                    cls_loss = cal_loss(outputs, labels, criterion) + cal_loss(outputs2, labels2, criterion)
                    if opt.kl_loss:
                ...
                if isinstance(preds, list) and isinstance(preds2, list):
                    ...
                    if opt.views == 2:
                        running_corrects2 += sum([float(torch.sum(pred == labels2.data)) for pred in preds2]) / len(
                            preds2)
                    else:
                        running_corrects2 += sum([float(torch.sum(pred == labels2.data)) for pred in preds2]) / len(
                            preds2)
            ...
            if epoch >= 50 and epoch_loss < min_loss:
                save_network(model, opt.name, epoch)
                min_loss = epoch_loss
            # draw_curve(epoch)
