...
from .Dataloader_SUES import Sampler_SUES, Dataloader_SUES, train_collate_fn
...
def make_dataset(opt):
    ...
    image_datasets = Dataloader_SUES(opt.data_dir, transforms=data_transforms)
    samper = Sampler_SUES(image_datasets, batchsize=opt.batchsize, sample_num=opt.sample_num,
                          triplet_loss=opt.triplet_loss)
    dataloaders = torch.utils.data.DataLoader(image_datasets, batch_size=opt.batchsize, sampler=samper, num_workers=0,
