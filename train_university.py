...
parser.add_argument('--name', default='CCR_Model_University', type=str, help='output model name')
...
parser.add_argument('--M', default=32, type=int, help='the CGB block output channel')
...
parser.add_argument('--model', default='convnext_small_22k_224', type=str, metavar='MODEL',
...
if not opt.resume:
    ...
    # record every run
    copyfile('train_university.py', dir_name + '/train_university.py')
            ...
            for data, data2, data3 in dataloaders:
                ...
                if phase == 'val':
                    ...
                    with autocast():
                        if opt.views == 2:
                            ...
                            outputs, outputs3, outputs2 = model(inputs, inputs2,
                                                                inputs3)  # satellite1 and drone2 and street3
                ...
                if opt.views == 2:
                    ...
                    if isinstance(outputs, list):
                        ...
                        cls_loss = cal_loss(outputs, labels, criterion) + cal_loss(outputs2, labels3,
                                                                                   criterion) + cal_loss(outputs3,
                                                                                                         labels2,
                                                                                                         criterion)
                        loss += cls_loss
                    else:
                ...
                if isinstance(preds, list) and isinstance(preds2, list):
                    ...
                    if opt.views == 2:
                        running_corrects2 += sum([float(torch.sum(pred == labels3.data)) for pred in preds2]) / len(
                            preds2)
                    else:
                        running_corrects2 += sum([float(torch.sum(pred == labels3.data)) for pred in preds2]) / len(
                            preds2)
                ...
                if opt.views == 3:
                    if isinstance(preds, list) and isinstance(preds2, list):
                        running_corrects3 += sum([float(torch.sum(pred == labels2.data)) for pred in preds3]) / len(
                            preds3)
                    else:
            ...
            if opt.views == 2:
                print(
                    '{} Loss: {:.4f} Cls_Loss:{:.4f} KL_Loss:{:.4f} Triplet_Loss {:.4f} Satellite_Acc: {:.4f}  Drone_Acc: {:.4f} lr_backbone:{:.6f} lr_other {:.6f}'
                        .format(phase, epoch_loss, epoch_cls_loss, epoch_kl_loss, epoch_triplet_loss, epoch_acc,
                                epoch_acc2,
                                lr_backbone, lr_other))
                with open(os.path.join('model', opt.name, opt.fname), 'a', encoding='utf-8') as f:
                    text = str(
                        '{} Loss: {:.4f} Cls_Loss:{:.4f} KL_Loss:{:.4f} Triplet_Loss {:.4f} Satellite_Acc: {:.4f}  Drone_Acc: {:.4f}'
                            .format(phase, epoch_loss, epoch_cls_loss, epoch_kl_loss, epoch_triplet_loss, epoch_acc,
                                    epoch_acc2)) + '\n'
                    f.write(text)
            elif opt.views == 3:
                ...
                print(
                    '{} Loss: {:.4f} Cls_Loss:{:.4f} KL_Loss:{:.4f} Triplet_Loss {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} Drone_Acc: {:.4f} lr_backbone:{:.6f} lr_other {:.6f}'
                        .format(phase, epoch_loss, epoch_cls_loss, epoch_kl_loss, epoch_triplet_loss, epoch_acc,
                                epoch_acc3,
                ...
                with open(os.path.join('model', opt.name, opt.fname), 'a', encoding='utf-8') as f:
                    text = str(
                        '{} Loss: {:.4f} Cls_Loss:{:.4f} KL_Loss:{:.4f} Triplet_Loss {:.4f} Satellite_Acc: {:.4f}  Street_Acc: {:.4f} Drone_Acc: {:.4f} lr_backbone:{:.6f} lr_other {:.6f}'
                            .format(phase, epoch_loss, epoch_cls_loss, epoch_kl_loss, epoch_triplet_loss, epoch_acc,
                                    epoch_acc3, epoch_acc2, lr_backbone, lr_other)) + '\n'
                    f.write(text)
