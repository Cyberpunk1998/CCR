...
from .ConvNext import make_CCR_model


class two_view_net(nn.Module):
    def __init__(self, class_num, block=4, M=32, return_f=False, resnet=False):
        super(two_view_net, self).__init__()
        self.model_1 = make_CCR_model(num_class=class_num, block=block, M=M, return_f=return_f, resnet=resnet)
...
class three_view_net(nn.Module):
    def __init__(self, class_num, share_weight=False, block=4, M=32, return_f=False, resnet=False):
        ...
        if self.share_weight:
            ...
            self.model_2 = make_CCR_model(num_class=class_num, block=block, M=M, return_f=return_f, resnet=resnet)
...
def make_model(opt):
    if opt.views == 2:
        ...
        model = three_view_net(opt.nclasses, share_weight=opt.share, block=opt.block, M=opt.M,
                               return_f=opt.triplet_loss, resnet=opt.resnet)
    return model
