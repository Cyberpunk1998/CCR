...
parser.add_argument('--name', default='CCR_Model_University', type=str, help='save model path')
...
if opt.multi:
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize, shuffle=False) for x in
                   ['gallery', 'query', 'multi-query']}
elif opt.part != '':
    image_datasets_query = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_query_transforms) for x in
                            ['query_drone']}
    image_datasets_gallery = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms) for x in
                              ['gallery_satellite']}
...
if __name__ == "__main__":
    ...
    os.system('python evaluate_university.py | tee -a %s' % result)
